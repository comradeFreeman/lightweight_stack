...
if __name__ == "__main__":
	...
	while True:
		...
		if defines.Codes.LIST[coap.code] == defines.Codes.NN_HANDSHAKE:
			...
			if isinstance(message, Request) and "test" in message.uri_path:
			...
			elif isinstance(message, Request) and "file" in message.uri_path and \
				...
				ser = build_message('NON', defines.Codes.NOISE_NN_RQ_ENCRYPTED, json.dumps(data, separators=(',', ':')),
						content_type = "application/octet-stream",	serialize = True, compression = Compression.gzip, coder = noise)
			else:
